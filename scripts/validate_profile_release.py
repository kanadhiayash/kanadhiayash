#!/usr/bin/env python3
"""Validate the dual-layer profile and approved samurai visual system."""
...
def validate(text: str, result: Result) -> None:
    ...
    if '<table role="presentation">' in text or "```mermaid" in text:
        result.error("Legacy card table or profile Mermaid diagram detected.")
    if TOOL_ROW not in text:
        result.error("Approved nine-tool row is missing.")
    if text.count('href="#60-second-profile"') < 2:
        result.error("Hero and static fallback must link to the 60-second profile.")
    ...
    for asset in (MOTION, STATIC, DECISION, *MEDIA):
        if not asset.is_file() or asset.stat().st_size == 0:
            result.error(f"Missing required asset: {asset.relative_to(ROOT)}")

    motion = MOTION.read_text(encoding="utf-8") if MOTION.is_file() else ""
    ...
    if "prefers-color-scheme: light" not in motion:
        result.error("Adaptive light mode is missing.")
    if "prefers-reduced-motion: reduce" not in motion:
        result.error("Reduced-motion handling is missing.")

    headings = {slug(m.group(2)) for m in HEADING.finditer(text)}
    for match in LINK.finditer(text):
        href = match.group("href")
        if href.startswith("#") and href[1:].lower() not in headings:
    ...
    result.ok("Copy, structure, ownership, assets, links, and hygiene checked.")
